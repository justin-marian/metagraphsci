"""
Disk caching for venue / publisher / author encoders.

Encoders are deterministic functions of the training-split document table, so
a JSON cache keyed on (seed, train-doc fingerprint) avoids the O(N) Polars
unique/sort work on every run.  The cache file is human-readable so vocabulary
shifts across runs can be diffed by hand.
"""
...
def compute_encoder_metadata(train_docs: pl.DataFrame, seed: int) -> dict[str, Any]:
    """Structural fingerprint of the inputs that determine encoder vocabularies."""
    return {
        "seed": int(seed),
...
def encoder_is_compatible(metadata: dict[str, Any], expected: dict[str, Any]) -> bool:
    """Validate that a saved encoder cache matches the current training split."""
    return all(metadata.get(k) == expected[k] for k in (
        "seed", "num_train_docs", "train_docs_fingerprint"
    ))
...
def load_encoder_cache(path: str | Path) -> tuple[Encoders, dict[str, Any]]:
    ...
    encoders: Encoders = {
        name: {str(k): int(v) for k, v in mapping.items()}
...
def build_encoder_cache(train_docs: pl.DataFrame) -> Encoders:
    """Build the three encoder vocabularies from the training-split documents."""
