...
REFERENCE_YEAR: float = float(YEAR)
YEAR_SCALE: float = 26.0
...
TEMPORAL_DECAY_YEARS: float = 5.0
#########################################################################################
...
OPENALEX_HEADERS: dict[str, str] = {"User-Agent": "MetaGraphSci/0.1 (research dataset builder)"}
...
OPENALEX_SELECT: str = ",".join([
    "id", "title", "abstract_inverted_index",
    "authorships", "primary_location", "publication_year",
...
# In bigger and lower volumes, the builder can afford a longer sleep to
# Keeps sustained throughput safely below the anonymous rate limit (~10 req/s).
OPENALEX_SLEEP_S: float = 0.12
OPENALEX_SLOW_SLEEP_S: float = 0.10

# Seconds to wait before a single automatic retry after a transient failure.
OPENALEX_RETRY_SLEEP_S: float = 3.0
# HTTP timeout for normal requests.
OPENALEX_TIMEOUT_S: int = 60
# HTTP timeout for the retry attempt.  Longer because the first
# attempt may have hit a transient overload that needs more time to clear.
OPENALEX_RETRY_TIMEOUT_S: int = 120
...
# A value of 3000–5000 typically raises avg degree from ~0.5 to 4–8× without
# significantly distorting the class distribution.
MAX_PAPERS: int = 3_000
...
#*
#*   Connectivity – rewards high-degree nodes (well-cited papers rank higher).
#*   Temporal     – rewards papers published close in time to the center node.
#*   Reciprocity  – rewards mutual citations (bidirectional edges).
#*   Overlap      – rewards papers that share many local neighbors (clustering).
CONNECTIVITY_WEIGHT: float = 0.35
TEMPORAL_WEIGHT:     float = 0.35
RECIPROCITY_WEIGHT:  float = 0.15
OVERLAP_WEIGHT:      float = 0.15
#########################################################################################

#########################################################################################
# Maximum incoming citations a node may have before being treated as a "hub"
# (survey paper, textbook) and excluded from neighbourhood scoring.
# Zero disables hub filtering; set to e.g. 500 to prune extremely dominant nodes.
HUB_DEGREE_THR: int = 0

# Number of BFS hops used to compute the structural hop-profile feature stored
# alongside each cache entry. 2 is a typical value for citation datasets,
# bigger hops produce richer profiles but require more expensive traversals
# during cache construction.
K_HOPS: int = 2

#! Spectral embedding dimension stored per cache entry. Zero disables spectral
#! computation (the default because eigendecomposition is expensive at scale).
SPECTRAL_DIM: int = 0

#! Eigendecomposition is only attempted on induced subgraphs smaller than this limit.
MAX_NODES_FOR_SPECTRAL: int = 25_000  #? BE CHANGED
...
MAX_GRAPH_NODES_FOR_HOPS: int = 25_000 #? BE CHANGED
#########################################################################################
...
SECONDARY_TOPIC_MULTIPLIER: float = 0.5
...
CONCEPT_SCORE_THR: float = 0.3

# --- Venue signal --

# Confidence for an exact venue-name match (strongest tier of venue evidence).
VENUE_EXACT_WEIGHT: float = 2.0

# Confidence for a partial / substring venue-name match.
VENUE_PARTIAL_WEIGHT: float = 1.0
#########################################################################################

#########################################################################################
# ------------------------- Global signal merge weights -------------------------
# Applied after individual signals are accumulated.  Higher weight = the
...
WEIGHT_VENUE:     float = 2.0
WEIGHT_OPENALEX:  float = 1.5
WEIGHT_LLM:       float = 1.2
...
# Max tokens expected in the JSON response (contains only label + confidence).
MAX_TOKENS: int = 64

# Maximum characters of abstract text forwarded to the LLM.  Keeps the
# request small while preserving sufficient context for a single-label guess.
...
LABELING_CONFIDENCE_THR: float = 0.45
...
BENCHMARK_DEFAULTS: dict[str, dict[str, Any]] = {
    "openalex_query": {
        ...
        # "transductive" exposes all nodes in every graph view (train/val/test) but masks labels appropriately.  
        # "inductive" is stricter - test nodes must never appear in the training graph, which requires 
        # more careful edge masking and generally produces lower baseline numbers.
        "graph_mode": "transductive",

        # Fraction of labeled nodes used for the semi-supervised training set.
        # Common ablation values: 0.05 (very low-label), 0.10, 0.20 (standard).
        "label_ratio": 0.10,
        "test_size": 0.20, "val_size": 0.10
    }
}
...
__all__ = [
    ...
    "OPENALEX_HEADERS",
    "OPENALEX_PAGE_SIZE",
    "OPENALEX_RETRY_SLEEP_S",
    "OPENALEX_RETRY_TIMEOUT_S",
    "OPENALEX_SLEEP_S",
    "OPENALEX_SLOW_SLEEP_S",
    "OPENALEX_TIMEOUT_S",
    ...
    "MAX_NODES_FOR_SPECTRAL",
    # --- DataLoader ---
    "DATALOADER_PREFETCH_FACTOR",
    # --- Labeling signal weights ---
    "LABELING_CONFIDENCE_THR",
    "ABSTRACT_MAX_CHARS",
    "CONFIDENCE_SCALE",
    "CONFIDENCE",
    "MAX_TOKENS",
